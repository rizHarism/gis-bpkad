...
<div class="container-fluid pb-5 ps-3 pe-3">
    <form id="edit-form" method="POST" class="form-horizontal" action="{{ $editable ? route('roles.update', ['role' => $role['id']]) : route('roles.store') }}">
        @method('PUT')
        {{ csrf_field() }}
        <div class="card-body">
            ...
            <div class="col-sm-10">
                <input type="text" name="name" class="form-control" id="name" placeholder="name" value="{{ $editable ? $role['name'] : "" }}">
            </div>
            </div>
            <div class="form-group row">
            ...
            <div class="col-sm-10">
                ...
                <div class="col-sm-4" style="margin-bottom: 20px;">
                    ...
                    <input name="permission[]" class="custom-control-input" type="checkbox" id="{{ $key . '-' . $permission['name'] . '-' . $i }}" value="{{ $permission['value'] }}" {{ $editable && in_array($permission['value'], $rolePermissions) ? 'checked="checked"' : '' }}>
                    <label for="{{ $key . '-' . $permission['name'] . '-' . $i }}" class="custom-control-label">{{ $permission['name'] }}</label>
                    </div>
                    @endforeach
                </div>
                @endforeach
    ...
    $(function() {
        $("#edit-form").submit(function() {
            $.ajax({
                ...
                permission: $(this).find("input:checkbox:checked").map(function(){return $(this).val();}).get()
                }),
                cache: false,
                contentType: false,
                processData: false,
